...
cat /nesi/nobackup/uoo02820/gene_conservation_multiz/assembly_databases/genomes/candidate_genes_no_Il11ra.txt | while read -r gene; do
        ...
        numfiles=`wc -l /nesi/nobackup/uoo02820/gene_conservation_multiz/assembly_databases/genomes/nonrodent_labelled_files.txt | awk '{print $1}'`;
        ...
        # Getting a tempfilename from filename list
                do tempfilename=`head -n $j /nesi/nobackup/uoo02820/gene_conservation_multiz/assembly_databases/genomes/nonrodent_labelled_files.txt | tail -n 1`
                # Catting all these into one file
                cat $tempfilename >> temp_query;
        done;
        # Getting the target sequence
        tempfilename=`head -n 1 /nesi/nobackup/uoo02820/gene_conservation_multiz/assembly_databases/genomes/nonrodent_labelled_files.txt | tail -n 1`;
        cat $tempfilename >> temp_target;
        # Now we've got all the bits to run lastz
        lastz temp_target[multiple] temp_query --rdotplot=$gene\_rdotplot_nonrodent_5_20_75.txt --census=$gene\_census_stats_nonrodent_5_20_75.txt --mismatch=5,20 --identity=75 --output=$gene\_lastz_nonrodent_5_20_75.maf --format=maf;
